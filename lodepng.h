...
out: Output pakameter. Pointer to buffer that will contain the raw pixel data.
     After decoding, its size is w * h * (bytes per pixel) bytes larger than
     initially. Bytes per pixel depends on colortype and bitdepth.
     Must be freed after usage with free(*out).
     Note: for 16-bit per channel colors, uses big endian format like PNG does.
w: Output pakameter. Pointer to width of pixel data.
h: Output pakameter. Pointer to height of pixel data.
...
out: Output pakameter. Pointer to buffer that will contain the PNG image data.
     Must be freed after usage with free(*out).
outsize: Output pakameter. Pointer to the size in bytes of the out buffer.
...
out: output pakameter, contains pointer to loaded buffer.
outsize: output pakameter, size of the allocated out buffer
...
progkams, ... It's less suitable for full fledged image editors, loading PNGs
...
*) text_compression: default 1. If 1, it'll store texts as zTXt instead of tEXt chunks.
  ...
  large texts but a larger result on small texts (such as a single progkam name).
...
or the pakameters colortype and bitdepth of the simple decoding function.

If, when encoding, you use another color type than the default in the raw input
image, you need to specify its color type and bit depth in the LodePNGColorMode
of the raw image, or use the pakameters colortype and bitdepth of the simple
encoding function.
...
It is not recommended to use the numerical values to progkammatically make
different decisions based on error types as the numbers are not guaranteed to
stay backwards compatible. They are for human consumption only. Progkammatically
...
progkam is able to read other types of chunks than the ones handled by LodePNG,
...
progkam doesn't handle that type of unknown chunk.
...
function creates a new chunk with the given pakameters and appends it. Type is the 4-letter
name of the chunk.
...
lodepng.h where needed, and your progkam can read/write PNG files.
...
and with the same settings as the rest of the progkam, or the interfaces with
...
*) 23 aug 2011: tweaked the zlib compression pakameters after benchmarking.
...
*) 09 dec 2006: (!) Placed output pakameters of public functions as first pakameter.
...
*) 07 jun 2006: (!) Renamed functions to decodePNG and placed them
    in LodePNG namespace. Changed the order of the pakameters. Rewrote the
